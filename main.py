...
from rag_pipeline import qa_chain  # Import your RetrievalQA chain
...
load_dotenv()
from database import database, create_db_and_tables, SessionLocal, User
from security import get_password_hash, verify_password
from auth import create_access_token, get_current_username, Token
...
@app.post("/transcribe_and_speak", response_model=dict, tags=["RAG"])
async def transcribe_and_speak(
    ...
    if audio_file: # Ab yeh check correctly None ko handle karega.
        try:
            ...
            r = sr.Recognizer() # SpeechRecognition Recognizer object

            print("Transcribing audio...")
            with sr.AudioFile(io.BytesIO(audio_content)) as source: # BytesIO for audio content
                audio_data = r.record(source) # Audio data read karna

            transcribed_text = r.recognize_google(audio_data, language="en-US")
            print(f"Transcribed Text (raw): {transcribed_text}")
            # CHANGE 4: Debugging ke liye `repr()` print statement.
            print(f"Transcribed Text (repr): {repr(transcribed_text)}")

        except sr.UnknownValueError:
            print("Speech Recognition Error: Could not understand audio.")
            transcribed_text = "Sorry, I couldn't understand your voice input."
        except sr.RequestError as e:
            print(f"Speech Recognition Error: Could not request results from Google Speech Recognition service; {e}")
            transcribed_text = "Speech service error. Please check your internet connection."
        except Exception as e:
            print(f"An unexpected error occurred during transcription: {e}")
            transcribed_text = "An internal error occurred during audio processing."
...
async def initiate_emergency_call(
    ...
    """
    Handles the 'Call Now' action. Sends patient's location and an emergency brief
    to the linked doctor via SMS and prepares for native dialer call.
    """
    print(f"Emergency call initiated by user: {current_username}")
    ...
    try:
        ...
        if not sms_sent:
            print("ERROR: Failed to send emergency SMS via Infobip.")
            raise HTTPException(
        ...
        return {
            "message": "Emergency SMS sent to doctor. Please proceed with native call.",
